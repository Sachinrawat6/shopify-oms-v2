...
//   'Billing Phone',
//   'Shipping Method',
//   'Total', // Order value, used for the >4000 rule
...
// // category: 'precancelled' | 'pending' | 'confirmed'
// //  - Blacklisted customers and "cancelled" tagged orders both land in
// //    'precancelled' for review -- the backend's blacklist guard still
// //    redirects the blacklisted ones into the Blacklisted Orders table on
// //    save, regardless of which endpoint they're posted to.
// //  - Draft orders (source = shopify_draft_order) are NOT a separate
// //    category. They still fall into confirmed/pending/precancelled like any
// //    other order, AND are additionally always sent to hold-orders too.
// //  - An order can carry more than one reason at once (e.g. both High RTO
// //    Risk and Bad Address) -- reasons are accumulated, not first-match-wins.
...
//   const baseConfirmed = financialStatus === 'paid';
...
//   // Priority 0: blacklisted customer -> always precancelled, no matter what
//   if (isBlacklisted) {
//     return { category: 'precancelled', reasons: ['Blacklisted Customer', ...draftNote], flags };
//   }
...
//   // Priority 2: RTO / bad address tags force the order to pending, even if
//   // confirmed. Priority 3: confirmed but order value exceeds the threshold.
//   // Both can apply at once, so reasons accumulate rather than short-circuit.
//   const forcedReasons = [];
...
//   if (baseConfirmed && isHighValue) {
//     forcedReasons.push(`Confirmed but amount exceeds ₹${HIGH_VALUE_COD_THRESHOLD}`);
//   }
...
//   if (baseConfirmed) {
//     return { category: 'confirmed', reasons: [...draftNote], flags };
//   }
...
//     const buckets = { precancelled: [], pending: [], confirmed: [], draftOrders: [] };
//     const counts = {
//       confirmed: 0,
//       pending: 0,
//       precancelled: 0,
...
//       if (result.flags?.baseConfirmed && result.flags?.isBadAddress) {
...
//     // Note: blacklisted rows are included in `precancelled` above (see
//     // classifyOrder) -- the backend's blacklist guard splits them out into
//     // the Blacklisted Orders table automatically when this batch is saved,
//     // so there's no separate "blacklisted" job needed here.
//     const jobs = [
...
//         })),
//         label: 'pre-cancelled (incl. blacklisted)',
//       },
//       {
//         resource: 'hold',
