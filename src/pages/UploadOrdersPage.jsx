...
// category: 'paymentPending' | 'blacklisted' | 'precancelled' | 'pending' | 'confirmed'
//
// UPDATED LOGIC:
// 0. Order ID present in paymentPendingIds (expired/pending payment list) -> ALWAYS Payment Pending
//    (checked before everything else, including blacklist)
// 1. Blacklisted -> Blacklisted Orders
...
const classifyOrder = (row, blacklistedCustomerIds, paymentPendingIds) => {
  ...
  const isPaymentPending = !!(paymentPendingIds && paymentPendingIds.has(orderId));
  ...
  // Priority 0 (highest): order id is in the payment-pending list -> ALWAYS PAYMENT PENDING
  // This is checked before blacklist/cancelled/paid so these orders are
  // pulled out of the normal flow entirely and routed to their own bucket.
  if (isPaymentPending) {
    return {
      category: 'paymentPending',
      reasons: ['Payment Pending', ...draftNote],
      flags,
    };
  }

  // Priority 1: blacklisted customer -> ALWAYS BLACKLISTED ORDERS
  if (isBlacklisted) {
    return {
  ...
  // Priority 2: cancelled tag -> precancelled
  if (isCancelled) {
    return { category: 'precancelled', reasons: ['Cancelled', ...draftNote], flags };
  }

  // Priority 3: IF ORDER IS PAID -> ALWAYS CONFIRMED
  // Regardless of RTO tags, bad address, or high value
  if (isPaid) {
  ...
  // Priority 4: NOT PAID - Check for RTO / Bad Address tags
  const forcedReasons = [];
  ...
  // Priority 5: NOT PAID and no RTO/Bad Address tags -> Pending with "Not Confirmed"
...
const categoryBadge = (category) => {
  const map = {
    ...
    paymentPending: 'bg-blue-100 text-blue-800',
  };
  const label = {
    confirmed: 'Confirmed',
    pending: 'Pending',
...
const UploadOrdersPage = () => {
  ...
  // Fast lookup set built from paymentPendingOrderIds, used by classifyOrder
  // to pull out orders whose "Name" (Order ID) matches an expired/pending
  // payment order.
  const paymentPendingIdSet = useMemo(
    () => new Set(paymentPendingOrderIds),
    [paymentPendingOrderIds]
  );
  ...
  const classifyAll = (rows) => {
    const buckets = {
      ...
      draftOrders: [],
      blacklisted: [],
      paymentPending: [], // New bucket for orders whose id is in the expired/pending payment list
    };
    const counts = {
      confirmed: 0,
      pending: 0,
    ...
    rows.forEach((row) => {
      const result = classifyOrder(row, blacklistedCustomerIds, paymentPendingIdSet);
      ...
      // Orders pulled into paymentPending are routed only to that bucket --
      // skip draft/RTO side-counters for them so the other summary numbers
      // stay accurate for the remaining orders.
      if (result.flags?.isPaymentPending) return;
  ...
  const sendOrdersToBackend = async () => {
    ...
    // Jobs array - payment-pending orders are routed to their own endpoint
    const jobs = [
      {
        resource: 'payment-pending/create',
        rows: buckets.paymentPending.map((o) => ({
          ...buildOrderPayload(o),
          payment_pending_reason: 'Order ID matched expired/pending payment list',
        })),
        ...
        <>
          <div className="bg-white rounded-lg shadow p-6 mb-6">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              ...
              <div className="bg-blue-100 p-4 flex flex-col justify-center items-center rounded-lg border border-blue-300">
                <div className="flex items-center gap-1">
                  <FiDollarSign className="text-blue-700" />
                  <span className="font-medium text-center text-sm">Payment Pending</span>
                </div>
                <p className="text-2xl font-bold mt-2 border-2 border-blue-700 w-12 h-12 grid items-center text-center rounded-full">
            ...
            <div className="overflow-x-auto h-[50vh]">
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {data.map((row, idx) => {
                    const { category, reasons } = classifyOrder(
                      row,
